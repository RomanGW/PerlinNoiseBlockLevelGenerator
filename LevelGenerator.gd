...
var block_mesh = block_base.instance().get_node("MeshInstance").mesh
...
func _generate_blocks():
	...
	for x in range(-map_size, map_size, 1):
		for z in range(-map_size, map_size, 1):
			...
			var perlin_value = perlin_noise.get_noise_2d(x * block_mesh.size.x + time, z * block_mesh.size.z)
			...
			new_block.translation = Vector3(x * block_mesh.size.x, vertical_offset * block_mesh.size.y, z * block_mesh.size.z)
